...
이 스펙을 처음 쓸 때는 좌측 사이드바가 있었지만, 그 사이 다른 작업으로 **TopBar(상단바) + 홈 화면 카드 그리드** 구조로 바뀌었다(`src/components/TopBar.tsx`, `src/app/(app)/page.tsx`). 채팅은 하루 중 계속 열어두고 쓰는 기능이라 홈 화면 카드 하나로 묻어두기보다, 모든 화면에서 바로 보이는 **TopBar에 상시 노출되는 채팅 아이콘/뱃지**로 만든다.

- `TopBar`에 💬 아이콘 버튼 추가 — 안읽은 방이 하나라도 있으면 합계 개수 배지를 표시. 클릭하면 `/chat`으로 이동(또는 드롭다운으로 방 목록만 먼저 보여주고 방을 고르면 `/chat/[roomId]`로 이동 — 구현 계획 단계에서 확정)
- `/chat` 페이지: 좌측에 방 목록(**토픽**·**채팅방** 2단 구성, JANDI와 동일한 분류, 안읽은 방은 굵게 표시 + 개수 배지), 우측에 선택한 방의 메시지 목록(위→아래 최신순 스크롤) + 하단 입력창(텍스트 + 파일 첨부 버튼)
- 방 목록 상단 "+" 버튼으로 새 방 만들기: 이름 / 종류(토픽·채팅) / 공개 여부(공개 또는 초대할 직원 선택)
- 방 목록 상단에 통합 검색창
...
- §9의 DB 레벨 차단은 **메시지 텍스트와 방 메타데이터**에는 완전히 적용되지만, 첨부파일은 구현을 단순하게 유지하려고 Storage 버킷을 public으로 두기로 했다 — URL 자체가 추측 불가능한 UUID 경로라 사실상 비공개지만, 그 URL을 알면 로그인 없이도 파일을 열 수 있다(메시지 텍스트처럼 RLS로 완전히 막히지는 않음). 더 엄격한 보호가 필요해지면 private 버킷 + signed URL로 전환한다.
